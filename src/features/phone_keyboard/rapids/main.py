...
def rapids_features(sensor_data_files, time_segment, provider, filter_data_by_segment, *args, **kwargs):
    ...
    if not keyboard_data.empty:
        ...
        if not keyboard_data.empty:
            ...
            keyboard_data["sessionStart"]           = keyboard_data["keyboardStrokeDuration"].apply(lambda x: 1 if x>=provider["NEW_SESSION_KEYSTROKE_MIN_PAUSE_LENGTH"] else 0)
            ...
            #print(keyboard_data.groupby(['local_segment', 'sessionNumber'])['local_timezone'].transform('count'))
            #print(len(keyboard_data.index))
            keyboard_data = keyboard_data[keyboard_data.groupby(['local_segment', 'sessionNumber'])['local_timezone'].transform('count') >  provider["MIN_KEYSTROKE_PER_SESSION"]]
            #print(len(keyboard_data.index))
