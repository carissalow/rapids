...
def run_barnett_features(input_dir, output_file, wtype="GLR", spread_pars=[10,1], tz="", center_rad=200, interval=10, acc_threshold=51.0, n_reps=1, min_pause_dur=300, min_pause_dist=60, r=None, w=None, tint_m=None, tint_k=None):
	...
	for i in range(n_reps):
		...
		output_features, slout, row_names = get_mobility_features(out3, obj3, mobmatmiss, tz, center_rad, interval)
...
def preprocessing(df, interval=10.0, acc_threshold=51.0, r=None,w=None,tint_m=None,tint_k=None):
	...
	df.columns = ['timestamp','UTC time','latitude','longitude','altitude','accuracy']
...
def sig_locs(mobmat, obj, center_rad=125, tz="", min_pause_time=600):
...
def day_dist(i1, i2, mobmat, subset_inds_v,subset_start_time_v, tz, center_rad):
...
def daily_routine_index(idx, mobmat, subset_inds_v, subset_day_of_week_v, subset_start_time_v, tz, center_rad):	
	...
	for i in range(len(subset_inds_v)):
		...
		daydist_v[i] = day_dist(idx, i, mobmat, subset_inds_v, subset_start_time_v, tz, center_rad)
...
def get_mobility_features(mobmat, obj, mobmatmiss, tz, center_rad, interval):
	print("Get mobility features...")	
	mobmat = np.array(mobmat)
	mobmatmiss = np.array(mobmatmiss)
	slout = sig_locs(mobmat, obj, center_rad, tz=tz) #x, y, timepresent, home
	IDhome = np.where(slout[:,3] ==1)[0]
	if len(IDhome) == 0:
		IDhome = 0
	...
	for i in range(len(daystr_v)):
		...
		if len(submat) == 0 or len(np.where(slout[:,3] ==1)[0]) == 0:
			...
			dri_output = daily_routine_index(i, mobmat, subset_inds_v, subset_day_of_week_v, subset_start_time_v, tz, center_rad)
