...
def apply_cluster_strategy(location_data, time_segment, clustering_algorithm, dbscan_eps, dbscan_minsamples, threshold_static, cluster_on, filter_data_by_segment):
    ...
    if cluster_on == "PARTICIPANT_DATASET":
        ...
        location_data = filter_data_by_segment(location_data, time_segment)
        location_data = cluster(location_data, clustering_algorithm, threshold_static, **hyperparameters)
        ...
        for segment_instance, instance_data in location_data.groupby(["local_segment"]):
            location_data_per_group = cluster(instance_data, clustering_algorithm, threshold_static, **hyperparameters)
...
def location_entropy(location_data):
    ...
    location_data = location_data.groupby(["local_segment", "cluster_label"])[["duration_in_seconds"]].sum().reset_index()
    location_data["duration_in_a_segment"] = location_data.groupby(["local_segment"])["duration_in_seconds"].transform("sum")
    location_data["plogp"] = (location_data["duration_in_seconds"] / location_data["duration_in_a_segment"]).apply(lambda x: x * np.log(x))
...
def doryab_features(sensor_data_files, time_segment, provider, filter_data_by_segment, *args, **kwargs):
    ...
    location_data = apply_cluster_strategy(location_data, time_segment, clustering_algorithm, dbscan_eps, dbscan_minsamples, threshold_static, cluster_on, filter_data_by_segment)
